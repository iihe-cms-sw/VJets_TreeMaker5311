...
bool SelectionUtils::DoIso2011(pat::ElectronCollection::const_iterator recoElectron, edm::Event& iEvent, std::vector< edm::Handle< edm::ValueMap<double> > > &IsoVals){
    ...
    // Effective area for 2012 Data and MC . From  https://twiki.cern.ch/twiki/bin/view/CMS/EgammaEARhoCorrection 
    if     (abs(recoElectron->eta())<=1.0)                   { A_eff_PH=0.117 ; A_eff_NH=0.014; }
    else if(abs(recoElectron->eta())>1.0 && abs(recoElectron->eta())<=1.479) { A_eff_PH=0.114 ; A_eff_NH=0.023; }
    else if(abs(recoElectron->eta())>1.479 && abs(recoElectron->eta())<=2.0) { A_eff_PH=0.049 ; A_eff_NH=0.019; }
    else if(abs(recoElectron->eta())>2.0 && abs(recoElectron->eta())<=2.2)   { A_eff_PH=0.077 ; A_eff_NH=0.012; }
    else if(abs(recoElectron->eta())>2.2 && abs(recoElectron->eta())<=2.3)   { A_eff_PH=0.089 ; A_eff_NH=0.018; }
    else if(abs(recoElectron->eta())>2.3 && abs(recoElectron->eta())<=2.4)   { A_eff_PH=0.083 ; A_eff_NH=0.028; }
    else                                     { A_eff_PH=0.099 ; A_eff_NH=0.039; }
...
std::vector<bool> SelectionUtils::MakePfEleNewIDAnalysis(pat::ElectronCollection::const_iterator recoElectron,edm::Event& iEvent,bool useNewID_,bool doWP90_,const edm::Handle<reco::ConversionCollection> &conversions,const reco::BeamSpot &beamspot,const edm::Handle<reco::VertexCollection> &vtxs, IsoDepositVals &IsoVals)
{
    ...
    if (!doWP90_){
        ...
        dEtaEBCut= 0.007;
        dEtaEECut= 0.009;
        hoeEBCut= 0.12; //0.12 in 2011?
        hoeEECut= 0.15; // 0.15 in 2011?
    } 
    ...
    float PFIsoPUCorrected =(charged + max(photon - lepIsoRho*A_eff_PH  , 0.) +  max(neutral - lepIsoRho * A_eff_NH, 0.))/std::max(0.5, recoElectron->pt());
    // seva: added line below because I'm not sure which is the proper pt to use for isolation. this should not be a big effect
    // but stil better to impose his cut on ntuple     
    //  reco::GsfElectronRef gsfele(els_h, elrecoElectron->originalObjectRef().key());
    //   cout<<"ecal pt"<< recoElectron->ecalDrivenMomentum().Pt()<<" pat pt:"<<recoElectron->pt()<<endl;
    if (recoElectron->pt()< recoElectron->ecalDrivenMomentum().Pt()) PFIsoPUCorrected =(charged + max(photon - lepIsoRho*A_eff_PH  , 0.) +  max(neutral - lepIsoRho * A_eff_NH, 0.))/std::max(0.5, recoElectron->ecalDrivenMomentum().Pt());
    ...
    // seva:  modified this a bit because I'm not sure which pt to use gsf or PF (they are different by at most 2-3 gev)
    // EGamma uses gsf pt, should we as well ?
    // I take the highest pt and use it for isoaltion
    if (PFIsoPUCorrected < 0.15) isIsolated= true;
