...
//  cout<<"------- NEW Event -----"<<endl;
  ...
  if (isElectron_){
    ...
    std::cout<<"input electron size:"<<electronCollection->size()<<std::endl;
...
//        cout<<"vec selection:"<<electronCollection->size()<<"     "<<recoElectron->pt()<<endl;	
	protection=true;
	jj++;
...
//	 cout<<recoElectron->charge()<<"   "<<recoElectron->pt()<<endl; 
	  ...
	  if (i>0){
		if (highestptele->pt()<recoElectron->pt()&&highestptele->charge()==recoElectron->charge()) highestptele=recoElectron;
		if (highestptele->charge()!=recoElectron->charge()){
				if (countSecond==0) {secondptele=recoElectron;countSecond=2;}
				else {
...
//	cout<<"IMA LI DVA:"<<countSecond<<"    "<<firstEleEnThrhold<<endl;
//      if (countSecond>1) cout<<"PT:"<<highestptele->pt()<<"    "<<secondptele->pt()<<"    "<<firstEleEnThrhold<<highestptele->charge()<<"    "<<secondptele->charge()<<endl; 
      if(countSecond>1 && (highestptele->pt()>=firstEleEnThrhold||secondptele->pt()>=firstEleEnThrhold)) { //you NEED at least two electrons :)
...
//	  eleSelStepByStep->SetBinContent(12,eleSelStepByStep->GetBinContent(12)+1); // (9) + Opposite Charge
	  ...
	  cout<<"electron pair mass:"<<e_ee_invMass<<" size:"<<sizePat<<endl; 
	  if (e_ee_invMass>highZmassLimit || e_ee_invMass<lowZmassLimit) passSelection=false;
	 //cout<<"sada sam tu   "<<passSelection<<"    "<<e_ee_invMass<<"    "<<delta<<endl; 
...
//	    if (i>2) h_zPt_3e->Fill(e_pair.Pt());
	    ...
	    for (reco::PFCandidateCollection::const_iterator pfElectron = pfElecCollection->begin (); pfElectron != pfElecCollection->end (); pfElectron++) {
	      if (fabs(pfElectron->pdgId())==11){
	        reco::GsfTrackRef PfTk= pfElectron->gsfTrackRef();
//		reco::GsfElectronRef gsfele=sfElectronRef();
	  //      reco::GsfTrackRef PfTk1= tag->gsfTrack();

		cout<<"nadji PF  "<<pfElectron->pt()<<"   "<<tag->pt()<<"    "<<probe->pt()<<"  "<<PfTk->pt()<<" tag gsf  "<< tag->gsfTrack()->pt()<<"probe gsf track:"<<probe->gsfTrack()->pt()<<" tag gsf "<<pfElectron->gsfElectronRef()->pt() <<endl;
...
//     reco::PFCandidate m1(lep1->charge(), lep1->p4() , reco::PFCandidate::mu);
    ...
    if (muonCollection.isValid () && muonCollection->size()>1) {
     // cout<<"SAD SAM U MUONIMA:"<<muonCollection->size()<<endl; 
       pat::MuonCollection::const_iterator highestptmu;
       pat::MuonCollection::const_iterator secondptmu,tempMuon;
	  ...
	  if (i==0) {highestptmu=recoElectron;
		counttempMuons++;}
	  if (i==1||(i>1&&counttempMuons==1)){
	    if (highestptmu->charge()!=recoElectron->charge()){counttempMuons++;
		...
		secondptmu=recoElectron;
	       ...
	       else{if (highestptmu->pt()<recoElectron->pt()) highestptmu=recoElectron;
		tempMuon=recoElectron;
		}
	  }
	  if (i>1&&counttempMuons>1){
	    
	     if (highestptmu->pt()<recoElectron->pt()&&highestptmu->charge()==recoElectron->charge());
	     if (secondptmu->pt()<recoElectron->pt()&&secondptmu->charge()==recoElectron->charge());
	  }
	  i++;
       }
	  ...
	  passSelection=false;
       ...
       if(counttempMuons>1                                                  //you NEED at least two electrons :)
	  ...
	  cout<<"muon pair mass:"<<mu_invMass<<" size:"<<sizePat<<endl; 
...
//  cout<<"size of pfnoelectrons:  "<<pOutput->size()<<endl;
  iEvent.put( pOutput );
