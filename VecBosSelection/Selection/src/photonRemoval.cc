...
photonRemoval::produce(edm::Event & iEvent, edm::EventSetup const & iSetup)
{
   ...
   vector<const reco::GenParticle*> eVectorTemp;
   eVector.clear();
   eVectorTemp.clear();
...
//    }
   ...
   if (particles.isValid ()){
      for (reco::GenParticleCollection::const_iterator it = particles->begin (); it != particles->end (); it++) {
//	 if (fabs(it->pdgId())==pdgIdLepton && it->status()==1 && it->eta() < maxElEta&&(fabs(it->mother()->pdgId())==pdgIdLepton||it->mother()->pdgId()==23||fabs(it->mother()->pdgId())==15)){
	 if (fabs(it->pdgId())==pdgIdLepton && it->status()==1 && it->eta() < maxElEta){
             //std::cout<<it->pdgId()<<"    "<<it->mother()->pdgId()<<"   "<<it->pt()<<std::endl;
	    // save the electrons
	    eVector.push_back(&(*it));
	 }
      }
   }
   //std::cout<<"REMOVE:"<<eVector.size()<<std::endl;
...
//   cout<<"MIN:"<<eMinusI<<"   "<<ePlusI<<"   "<<(eMinusP4+ePlusP4).M()<<endl;
...
//if ( eMinusI!=-1){ cout<<eVector.size()<<"  "<<eMinusI<<"   "<<ePlusI<<"  "<<eVector[eMinusI]->pt()<<endl; }
//if ( ePlusI!=-1){ cout<<eVector.size()<<"  "<<eMinusI<<"   "<<ePlusI<<"  "<<eVector[ePlusI]->pt()<<endl; }
...
//               cout<<"NASAO SAM MUON:"<<partB->pt()<<endl;
           if ((ePlusI!=-1||eMinusI!=-1 )&&S1>0) {
//		cout<<"tetst:"<<partB <<"   "<<eVector[ePlusI]<<"   "<<eVector[eMinusI]<<"     "<<endl;
...
//  cout<<"uzeo sam van:  "<<eVector.size()<<"   "<<S1<<"   "<<S2<<endl;
   iEvent.put( pOutput );
