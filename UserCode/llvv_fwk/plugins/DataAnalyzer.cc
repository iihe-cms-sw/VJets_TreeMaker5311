...
DataAnalyzer::DataAnalyzer(const edm::ParameterSet &iConfig) : obsPU_h(0), truePU_h(0)
{
    ...
    vector<string> trigs=analysisCfg_.getParameter<vector<string> >("triggerPaths");
    vector<string> filts=analysisCfg_.getParameter<vector<string> >("metFilters");
    ...
    for (size_t istep(0); istep < sizeof(selSteps)/sizeof(string); istep++) 
    {
        cutflow_h->GetXaxis()->SetBinLabel(istep + 1, selSteps[istep].c_str());
    }

    trigger_h = fs->make<TH1D>( "trigger",    ";Trigger path;",       trigs.size(),0,trigs.size());
    for (size_t itrig=(0); itrig < trigs.size(); itrig++) 
    {
        trigger_h->GetXaxis()->SetBinLabel(itrig + 1, trigs[itrig].c_str());
    }

    filter_h = fs->make<TH1D>( "filter",    ";Filter bits;",       filts.size(),0,filts.size());
    for (size_t ifilt=(0); ifilt < filts.size(); ifilt++) 
    {
        filter_h->GetXaxis()->SetBinLabel(ifilt + 1, filts[ifilt].c_str());
    }
...
void DataAnalyzer::analyze(const edm::Event &event, const edm::EventSetup &iSetup) 
{
    ...
    if (!isData)
    {
        ...
        if (puInfoH.isValid())
        {
            for(vector<PileupSummaryInfo>::const_iterator it = puInfoH->begin(); it != puInfoH->end(); it++)
            {
                if (it->getBunchCrossing() == 0) 
                {
                    ev.ngenITpu += it->getPU_NumInteractions();
                    ev.ngenTruepu = it->getTrueNumInteractions();
                }
                else if (it->getBunchCrossing() < 0) 
                { 
                    ev.ngenOOTpum1 += it->getPU_NumInteractions(); 
                }
                else { 
                    ev.ngenOOTpu += it->getPU_NumInteractions(); 
                }
            }
        }
        ...
        for (size_t i(0); i < genParticlesH->size(); ++i)
        {
            ...
            if (DEBUG && (p.pdgId() == 11 || p.pdgId() == 13 || p.pdgId() == 15) && p.status() == 1) 
            {
                cout << "genSt1 " << p.pdgId() << " " << p.mother()->pdgId() << " " << p.pt() << " " << p.eta() << " mother is " << p.mother()->pdgId() << endl;
            }
            if (DEBUG && (p.pdgId() == 11 || p.pdgId() == 13 || p.pdgId() == 15) && p.status() == 3) 
            {
                cout << "genSt3 " << p.pdgId() << " " << p.mother()->pdgId() << " " << p.pt() << " " << p.eta() << " mother is " << p.mother()->pdgId() << endl;
            }
            ...
            if (leptonIsOkToAccept)
            {
                ...
                else 
                {
                    ...
                    if(p.numberOfMothers() == 1 &&  p.mother()->pdgId() != p.pdgId() && p.mother()->pdgId() != 23 && abs(p.mother()->pdgId()) != 24) 
                    {
                        leptonIsOkToAccept=false;
                    }
                    //sherpa-like
                    if(p.numberOfMothers() == 2 && (abs(p.mother(0)->pdgId()) != abs(p.pdgId()) || abs(p.mother(1)->pdgId()) != abs(p.pdgId()))) 
                    {
                        leptonIsOkToAccept=false;
                    }
                }
            ...
            ev.mc_lxy[ev.mcn]=0;
            if ((p.pdgId() == 11 || p.pdgId() == 13) && p.status() == 1 && (abs(p.mother()->pdgId()) == abs(p.pdgId()) || p.mother()->pdgId() == 23)) {
                cout << ev.event << " gen Lep: " << p.pdgId() << " mother ID: " << p.mother()->pdgId() << " " << p.pt() << " " << p.eta() << endl;
            }
    ...
    vector<string> triggerPaths = analysisCfg_.getParameter<vector<string> >("triggerPaths");
    vector<int> triggerCats     = analysisCfg_.getParameter<vector<int> >("triggerCats");
    ...
    for (size_t itrig(0); itrig != triggerBitsH->size(); ++itrig)
    {
        ...
        {
            if (trigName.find(triggerPaths[it]) == string::npos) continue;
            ev.t_bits[it] = true;
    ...
    for (size_t imu(0); imu < muH->size(); ++imu)
    {
        ...
        bool isTight(isPF 
                && isGlobal              
                && fabs(ev.ln_d0[ev.ln]) < 0.2 
                && fabs(ev.ln_dZ[ev.ln]) < 0.5 
                && ev.ln_trkValidPixelHits[ev.ln] > 0  
                && ev.ln_trkchi2[ev.ln] < 10. 
                && ev.mn_validMuonHits[ev.mn] > 0.  
                && ev.mn_nMatchedStations[ev.mn] > 1
                && ev.mn_trkLayersWithMeasurement[ev.mn] > 5);

        bool isSoft(isTracker 
                && muon->muonID("TMOneStationTight") 
                && fabs(ev.ln_d0[ev.ln]) < 3.
                && fabs(ev.ln_dZ[ev.ln]) < 30.
                && ev.mn_trkLayersWithMeasurement[ev.mn] > 5 
                && ev.mn_pixelLayersWithMeasurement[ev.mn] > 1  
                && ev.mn_innerTrackChi2[ev.mn] < 1.8);
    ...
    if (!isData && keepFullGenInfo_ ) 
    {
        ...
        for (; gjeti!=genJetColl->end(); gjeti++) 
        {
            ...
            vector<const GenParticle*> mcparticles = gjeti->getGenConstituents();
            for (vector<const GenParticle*>::const_iterator thepart = mcparticles.begin(); thepart != mcparticles.end(); ++thepart)
            {
                if ((**thepart).charge() != 0) 
                {
                    isChargedJet = true;
    ...
    //
    vector<edm::InputTag> metSources = analysisCfg_.getParameter<vector<edm::InputTag> >("metSource");
    ev.metn = 0;
