...
class PFRecoTauDiscriminationAgainstElectronMVA3GBR : public PFTauDiscriminationProducerBase  {
public:
  explicit PFRecoTauDiscriminationAgainstElectronMVA3GBR(const edm::ParameterSet& iConfig)
      ...
      category_output_(0)
  {
    ...
    if ( !gbrFile_.isLocal()) throw cms::Exception("PFRecoTauDiscriminationAgainstElectronMVA3GBR") 
    ...
    mva_ = new AntiElectronIDMVA3GBR();
...
private:
  ...
  {
    //std::cout << "<PFRecoTauDiscriminationAgainstElectronMVA3GBR::readZippedFile>:" << std::endl;
...
void PFRecoTauDiscriminationAgainstElectronMVA3GBR::beginEvent(const edm::Event& evt, const edm::EventSetup& es)
{
  if ( returnMVA_ ) {
...
double PFRecoTauDiscriminationAgainstElectronMVA3GBR::discriminate(const PFTauRef& thePFTauRef)
{
  double mva = 1.;
...
void PFRecoTauDiscriminationAgainstElectronMVA3GBR::endEvent(edm::Event& evt)
{
...
bool
PFRecoTauDiscriminationAgainstElectronMVA3GBR::isInEcalCrack(double eta) const
{
  eta = fabs(eta);
  return (eta>1.460 && eta<1.558);
}

DEFINE_FWK_MODULE(PFRecoTauDiscriminationAgainstElectronMVA3GBR);
